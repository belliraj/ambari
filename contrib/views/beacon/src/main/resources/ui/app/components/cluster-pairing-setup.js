...
export default Ember.Component.extend({
  ...
  getTargetClusterInfo(targetCluster) {
    ...
    return new Ember.RSVP.Promise((resolve, reject) => {
      this
        .get('beaconViewService')
        .getRemoteClusterInfo(
          remoteAmbariUrl,
          {
            'userName': targetCluster.get('username'),
            'password': targetCluster.get('password')
          }
        ).done((clusterInfo) => {
          clusterInfo.beaconEndpoint = remoteAmbariUrl.substr(0, targetCluster.get('url').indexOf('8080')) + '25000';
      ...
      this.getClustersRegisteredInTarget(targetClusterInfo).then((clustersRegisteredInTarget) => {
        if (!clustersRegisteredInTarget.cluster.findBy('name', this.get('localAmbariCluster.name'))) {
          ...
          this.get('remoteBeaconService')
            .registerCluster(
              this.get('localAmbariCluster.name'),
              this.extractRegistrationInfo(this.get('localAmbariCluster')),
              targetClusterInfo.beaconEndpoint
            )
            .done(() => {
              resolve();
              this.sendAction('update');
  ...
  pairClusters(targetClusterInfo) {
    return new Ember.RSVP.Promise((resolve, reject) => {
      ...
      this.get('beaconService')
        .pairClusters(targetClusterInfo.name, targetClusterInfo.beaconEndpoint)
        .done(() => {
